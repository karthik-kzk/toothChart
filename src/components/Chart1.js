...
import wireButtonIcon from "../images/Icons/wireIcon.jpg";
import wireIcon from "../images/Icons/wireIcon.png";
...
function Chart() {
  ...
  function cavityFunction() {
    ...
    for (var i in selected) {
      if (selected[i]) {
        const elementIndex = logComment.findIndex((e) => e.name === i);
  ...
  // toggle function to draw straight line function CANVAS
  function deleteRange(logName, deleteSolution) {
    let logComment = [...log.logs];
    // to get the  range of  array
    // getting start value
    let startIndex = logComment.findIndex((e) => e.name == logName);
    let array = [];
    if (deleteSolution === "wire") {
      ...
      for (let i = 0; i < array.length; i++) {
        // if (array !== undefined) {
        const elementIndex = logComment.findIndex((e) => e.name == array[i]);
        logComment[elementIndex] = {
          ...logComment[elementIndex],
          wire: false,
      ...
      let logComment = [...log.logs];
      const elementIndex = logComment.findIndex((e) => e.name == logName);
      logComment[elementIndex] = {
        ...logComment[elementIndex],
        rubberBand: false,
        // coiledSpring: false,
  ...
  //  cancel range selection or select box
  function cancelSelectBox() {
    setShowBracketType(false);
  ...
  function deleteLog(logName, deleteSolution) {
    let logComment = [...log.logs];
    if (deleteSolution === "wire") {
      const elementIndex = logComment.findIndex((e) => e.name == logName);
      logComment[elementIndex] = {
    ...
    } else if (deleteSolution === "cavity" || deleteSolution === "implant") {
      const elementIndex = logComment.findIndex((e) => e.name == logName);
      logComment[elementIndex] = {
        ...logComment[elementIndex],
        value: 0,
  ...
  function editLog(logName) {
    // let logComment = [...log.logs];
    // const elementIndex = logComment.findIndex(
    //   (e) => e.name == Object.keys(logName)
    // );
    // if (Object.values(logName) == "cavity") {
    //   console.log("cavity");
    //   logComment[elementIndex] = {
    //     ...logComment[elementIndex],
    //     value: 1,
    //     log: `${Object.values(logName)}`,
    //   };
    // } else if (Object.values(logName) == "implant") {
    //   logComment[elementIndex] = {
    //     ...logComment[elementIndex],
    //     value: 2,
    //     log: `${Object.values(logName)}`,
    //   };
    // } else if (Object.values(logName) == "function3") {
    //   logComment[elementIndex] = {
    //     ...logComment[elementIndex],
    //     value: 3,
    //     log: `${Object.values(logName)}`,
    //   };
    // } else if (Object.values(logname) == "function4") {
    //   logComment[elementIndex] = {
    //     ...logComment[elementIndex],
    //     value: 4,
    //     log: `${Object.values(logName)}`,
  ...
  const upperTeeth = log.logs.slice(0, 16).map((log, i) => {
    return (
      <div className="column" key={i}>
        ...
        <div
          className={`tooth_and_line  ${selected[log.name] ? "shadow" : " "}`}
        >
          <input
            type="image"
  ...
  const lowerTeeth = log.logs.slice(16, 32).map((log, i) => {
    return (
      <div className=" column" key={i}>
        <div className={` ${selected[log.name] ? "shadow" : " "}`}>
          <input
            type="image"
            src={teeth[log.name][log.value]}
            style={toothStyle}
  ...
  const chainTypeButtons = (
    <div>
      <div className="button-column button3" onClick={chainFunction}>
        {/* chain type 1 */}
        <input type="image" src={teeth.chain[0]} style={buttonStyle} alt="0" />
      </div>
      <div className="button-column button3" onClick={chainFunction}>
        {/*chain type 2 */}
        <input type="image" src={teeth.chain[1]} style={buttonStyle} alt="1" />
      </div>
      <div className="button-column button3" onClick={chainFunction}>
        {/* chain type 3 */}
        <input type="image" src={teeth.chain[2]} style={buttonStyle} alt="2" />
      </div>
    </div>
  );
  ...
  useEffect(() => {    
    axios
      .post(
        "http://ortho.zbapps.in/admin/api/log-update",
  ...
  useEffect(() => {
    axios
      .get("http://ortho.zbapps.in/admin/api/log-list")
    ...
    <div className="all">
      <div className="chart-sticky">
        ...
        <div className="teeth-plus-button">
          ...
          <div className="button-row vertical_scroll">
            ...
            <div className="button-column button6" onClick={rubberBandFunction}>
              {/* RubberBand */}
              <input
                type="image"
                src={teeth.icons[5]}
        ...
        {/* <div className={`canvas ${enableCanvas ? " " : " disable-user-select"}`}> */}
        <div className="canvas ">
          <div className="canvas-parent">
            ...
            <div className={`${enableCanvas?"":"visibility_hidden"}`}>
            {/* {enableCanvas ? ( */}
              <Canvas
      ...
      <div className="log-table">
        ...
        <LogTable
          ...
          //solution is bracket or coil applied
        />
